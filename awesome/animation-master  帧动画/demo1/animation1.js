...
/***/ (function(module, exports, __webpack_require__) {
	...
	Animation.prototype.changePosition = function (ele, positions, imageUrl) {
		...
		if (len) {
			...
			taskFn = function (next, time) {
				...
				//获得当前背景图片位置索引 time / me.interval | 0 相当于Math。floor整数的时候
	...
	Animation.prototype.repeat = function (times) {
		...
		var taskFn = function () {
			...
			if (times) {
				...
				//达到重复执行次数，则跳转到下一个任务 _next 方法会将this.index++
	...
	Animation.prototype.restart = function () {
		if (this.state === STATE_STOP) {
			this.state = STATE_START;
			// console.log(this.index, 'restart');
	 ...
	 * 重新开始执行当前异步定时任务
	 */
	Animation.prototype.jump = function (index) {
		if (this.state === STATE_STOP) {
			this.state = STATE_START;
			this.index = index
			this._runTask()
	...
	Animation.prototype._next = function (task) {
		...
		task.wait ? this.waitingTime = setTimeout(function () {
			me._runTask();
			// console.log('_runTask', me.index);
...
/***/ (function(module, exports) {
	 ...
	 * 动画停止
	 */
	Timeline.prototype.stop = function (index) {
		if (this.state !== STATE_START)
			return;
		this.state = STATE_STOP;
	...
	function startTimeline(timeline, startTime) {
		...
		function nextTick() {
			...
			if (now - lastTick >= timeline.interval) {
				// console.log(timeline.onenterframe);
